...
void erase(int x) {                   
  Vertex* erase_vertex = NULL;
	erase_vertex = find(root, x);
	if (erase_vertex == NULL || erase_vertex->key != x)
		return;
	if(erase_vertex->left != NULL)
		erase_vertex->left->parent = NULL;
	if(erase_vertex->right != NULL)
		erase_vertex->right->parent = NULL;
	root = merge(erase_vertex->left,erase_vertex->right);

}

bool find(int x) {  
	Vertex* find_vertex = NULL;
	find_vertex = find(root, x);
	if (find_vertex != NULL && find_vertex->key == x)
		return true;
	return false;
}
